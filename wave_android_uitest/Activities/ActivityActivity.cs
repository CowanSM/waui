...
//			MenuInflater.Inflate (Resource.Menu.activity_actions, menu);
		...
		protected override void OnCreate (Bundle bundle) {
			...
			transaction.Add (Resource.Id.activity_root, fragment);
			transaction.Commit ();

			_filterOptions = FilterOptions.ALL;

			((RelativeLayout.LayoutParams)FindViewById<ViewPager>(Resource.Id.activity_pager).LayoutParameters).AddRule (LayoutRules.Above, Resource.Id.activity_tab_bar);
			...
			adaptor.AddFragmentView ((i, v, b) => {
				...
				/* Populate activity list here... For now example: */
				Android.Text.SpannableString span = new Android.Text.SpannableString ("Oscar Bluth challenged Lucille Bluth.");
				span.SetSpan(new Android.Text.Style.TextAppearanceSpan(BaseContext, Android.Resource.Style.TextAppearanceDeviceDefaultWidgetActionModeTitle), 0,11, Android.Text.SpanTypes.InclusiveInclusive);
				...
				}), 0, 11, Android.Text.SpanTypes.InclusiveInclusive);
				span.SetSpan (new PersonSpan(() => {
					var intent = new Intent(BaseContext, typeof(PersonActivity));
					intent.PutExtra("person_name", "Lucille Bluth");
					StartActivity(intent);
				}), 23, 35, Android.Text.SpanTypes.InclusiveInclusive);
                listAdaptor.AddItem(new ActivityListAdapter.Item() {
                    ImageResLeft = Resource.Drawable.person_outline,
                    ImageResRight = Resource.Drawable.person_outline,
                    TopSequence = span,
                    BottomText = "Most steps in 2 hours. Bet: Dinner and drinks.",
                    TimeText = "5 min ago"
                });
				/* * * * * * * * * * * * */
